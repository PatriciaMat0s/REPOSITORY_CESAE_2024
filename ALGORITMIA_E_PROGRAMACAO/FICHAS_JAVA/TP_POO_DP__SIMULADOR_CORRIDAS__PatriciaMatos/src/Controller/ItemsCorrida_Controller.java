...
import Domain.Veiculo.Carro; import Domain.Veiculo.Mota;
...
//
     ...
     * @param piloto
    //* @param numItemEscolhido é o numero da opçao introduzida pelo jogador, lida da consola
     */
...
//IMPRIMIR INVENTÁRIOS DE ITEMS:
        ...
        if (veiculoAtual instanceof Mota) { //CASO O VEICULOATUAL SEJA MOTA:
            ...
            // Perguntar qual item quer usar e aplicar os efeitos do item escolhido no Veiculo do Piloto:

            Habilidade habilidadeAUsar;
            habilidadeAUsar = (((Mota) veiculoAtual).getHabilidadesMota()).get(numItemAUsar-1);


            //Reinstanciar a classe veiculo com os upgrades de aplicar o item, ou seja, reinstanciar a Mota:

            //1. Quero conservar os atributos que o veiculo actaul tem, apenas quero mudar os que vao ser influenciados pela aplicaçaoo do item:
            String marca = ((Mota) veiculoAtual).getMarca();
            String modelo = ((Mota) veiculoAtual).getModelo();
            double preco = ((Mota) veiculoAtual).getPreco();

            //2. Já os atributos vao beneficiar da aplicaçao do item
            ...
            //Finalmente:
            Veiculo veiculoAtual = new Mota(marca, modelo, potencia, peso, desgaste, preco);

        }

       ACABAR if (veiculoAtual instanceof Carro) {
