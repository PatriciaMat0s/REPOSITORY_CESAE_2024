...
SELECT * /*ou: SELECT nome, cidade, data_inscricao, pontos*/ 
FROM Cliente
...
FROM Cliente
GROUP BY cidade 
HAVING count(nome) > 1 /*WHERE usa-se so qdo ha condiçoes sem funcçoes de agregaçao - o count() é!. Se as há usa se o HAVING*/
...
-- !!!Com a materia dos ultimos slides das QUERIES (DQL):
